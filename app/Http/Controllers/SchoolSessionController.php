    ...
    */
    ...
    public function store(SchoolSessionStoreRequest $request)
    {
        try {
            ...
            return back()->with('status', 'La creación de la sesión ha sido un éxito.!');
        } catch (\Exception $e) {
    ...
    public function browse(SchoolSessionBrowseRequest $request)
    {
        try {
            ...
            return back()->with('status', '¡El conjunto de sesiones de exploración se ha realizado correctamente!');
        } catch (\Exception $e) {
            return back()->withError($e->getMessage());
        }
