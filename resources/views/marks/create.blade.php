            ...
            <div class="row pt-2">
                <div class="col ps-4">
                    <h1 class="display-6 mb-3">
                        <i class="bi bi-cloud-sun"></i> {{__('Give Marks')}}
                    </h1>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{route('home')}}">{{__('Home')}}</a></li>
                            <li class="breadcrumb-item"><a href="{{url()->previous()}}">{{__('My courses')}}</a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{__('Give Marks')}}</li>
                        </ol>
                    </nav>
                    @include('session-messages')
                    @if ($academic_setting['marks_submission_status'] == "on")
                    <p class="text-primary">
                        <i class="bi bi-exclamation-diamond-fill me-2"></i> {{__('Marks Submission Window is open now.')}}
                    </p>
                    @endif
                    <p class="text-primary">
                        <i class="bi bi-exclamation-diamond-fill me-2"></i> {{__('Final Marks submission should be done only once in a Semester when the Marks Submission Window is open.')}}
                    </p>
                    @if ($final_marks_submitted)
                    <p class="text-success">
                        <i class="bi bi-exclamation-diamond-fill me-2"></i> {{__('Marks are submitted.')}}
                    </p>
                    @endif
                    <h3><i class="bi bi-diagram-2"></i> {{__('Class')}} #{{request()->query('class_name')}}, Section #{{request()->query('section_name')}}</h3>
                    ...
                    <form action="{{route('course.mark.store')}}" method="POST">
                        ...
                        {{-- <div class="row justify-content-between mb-3"> --}}
                            ...
                            @else
                                ...
                                <div class="col-5">
                                    <p class="text-success">
                                        <i class="bi bi-exclamation-diamond-fill me-2"></i> {{__('You have submitted Final Marks')}} <i class="bi bi-stars"></i>.
                                    </p>
                                </div>
                                @else
                                <div class="col-5">
                                    <p class="text-primary">
                                        <i class="bi bi-exclamation-diamond-fill me-2"></i> {{__('Create Exam to give marks.')}}
                                    </p>
                                </div>
                                @endif
                            @endif
